---
title: "Coursera - Peer-graded Assignment: Regression Models Course Project"
...
To provide an overview about the data, the Table below shows the first 6 columns of data.  
...
With this details, is easy to figure out what columns we need to use for our main analysis. *mpg* and *am*.  
...
The **Figure #1**  shows the distribution between Automatic (`r nrow(aut)`) and Manual (`r nrow(man)`) transmission and the **Figure #2** shows a Boxplot that compares the Automatic x Manual transmission for MPG. Also the **Figure #3** shows the Exponential means distribution for both. Those pictures are the first evidence that trasmissions are a significant variable for MPG. It´s possible to see that Manual transmission cars have a better performance, **`r round(mean(man$mpg),2)` mpg** than Automatic transmission with **`r round(mean(aut$mpg),2)` mpg**.  
...
x <- summary(lm(mpg ~ am, data = mtcars))$coeff
x # use x due to add results into the text
```
The first linear regression analysis, model with just transmission variable, shows that manual transmission have about `r round(x[2,1],2)` MPG better performance than automatic transmission vehicles. The intercept results is the same from the means, `r round(x[1,1],2)` for automatic and `r round(x[1,1],2)` + `r round(x[2,1],2)`= `r round(x[1,1]+x[2,1],2)` for manual transmissions.  
But according to the next linear regression analysis, it´s possible to see that transmission variable (am) contribute less to MPG compared to the others variables.
...
```
Using **Variance Inflation Factor** (vif), we can see that some of the variables are quite high than *am* (transmission), like as *disp* variable (`r vif_result[2]`), *cyl* (`r vif_result[1]`), *wt* (`r vif_result[5]`) etc. that is higher than the instance when it were orthogonal to all the other regressors.  

Also to confirm that, it was pointed out a correlation table with the others variables where we can see that have higher significative variables  to MPG outcome. Take a look below.  
...
To help to understant it better, it was provided some charts, **Figure #4** that shows also this corrrelations.

# **`r colorize("Conclusion", "green")`** 

In a single view, comparing just the Mpg versus Transmission, we can assure with 95% of confidence that Manual Transmission have a better performance than Automatic Transmission.  But we can not forget that we found a couple of variables that have a higher correlation with **mpg** than **ma** vars.  
Some examples are in **Figure #4**, that show us some important points that we need to consider.  
...
mtcars %>% ggplot(aes(x = mpg))+
  ...
  args = list(mean = mean(man$mpg), sd = sd(man$mpg)),
  color="blue", size =1)+
...
Diagnostic QQ plot of standardized residuals.  
